        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header text-center">
                    <h1 class="h4 mt-5 text-gray-800">Point Of Sales | Periode Rerport Report</h1>
                    <p class="text-gray-800" st>JL.Ki Maja GG Cempaka No 43, Kedaton Bandar Lampung</p>
                    <p class="mt-0 text-gray-800"><?= $from . ' - ' . $to ?></p>
                </div>
                <div class="card-body">
                    <div class="table">
                        <table class="table table-bordered text-center text-kecil" id="" width="" cellspacing="10">
                            ...
                            <tbody>
                                ...
                                //loop data
                                $no = 1;
                                while ($selPeriodd  = $selPeriodx->fetch_object()) { ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $selPeriodd->noInvoice ?></td>
                                        <td><?= $selPeriodd->fullName ?></td>
                                        <td><?= $selPeriodd->qtyTotal ?></td>
                                        <td>Rp.<?= number_format("$selPeriodd->grandTotal ", 0, ",", ".") ?></td>
                                        <td>Rp.<?= number_format("$selPeriodd->pay ", 0, ",", ".") ?></td>
                                        <td>Rp.<?= number_format("$selPeriodd->change ", 0, ",", ".") ?></td>
                                        <td><?= $selPeriodd->inputDate ?></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
